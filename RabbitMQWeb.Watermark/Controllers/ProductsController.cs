...
namespace RabbitMQWeb.Watermark.Controllers
{
    public class ProductsController : Controller
    {
        ...
        public async Task<IActionResult> Create([Bind("Id,Name,Price,Stock,ImageName")] Product product,IFormFile ImageFile)
        {
            ...
            {
                var randommImageName=Guid.NewGuid()+Path.GetExtension(ImageFile.FileName);
                var path = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot/images", randommImageName);
                await using FileStream stream = new(path, FileMode.Create);

                await ImageFile.CopyToAsync(stream);

                _rabbitMQPublisher.Publish(new productImageCreatedEvent() { ImageName = randommImageName });

                product.ImageName = randommImageName;
            }
